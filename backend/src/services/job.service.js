...
class JobService {
  ...
  async updateJobStatus(jobId, companyId, status) {
    ...
    await writeTransaction((tx) =>
      tx.run("MATCH (j:Job {id:$id}) SET j.status = $status", {
        id: jobId,
        status,
      }),
    ).catch(() => {});
  ...
  async searchJobs(filters, paginationQuery) {
    ...
    if (filters.salaryMin)
      mongoFilter["salary.max"] = { $gte: parseInt(filters.salaryMin) };
    if (filters.salaryMax)
      mongoFilter["salary.min"] = { $lte: parseInt(filters.salaryMax) };
    if (filters.industry)
      mongoFilter["companyInfo.industry"] = new RegExp(filters.industry, "i");
    if (filters.skills) {
  ...
  async getJobDetail(jobId, userId) {
    ...
    if (!job) {
      const err = new Error("Tin tuyển dụng không tồn tại.");
      err.statusCode = 404;
      throw err;
    }
  ...
  async _syncJobToNeo4j(
    jobId,
    { title, level, location, salaryMin, salaryMax, requirements },
  ) {

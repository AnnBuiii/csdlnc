...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await appService.apply(req.user.candidateId, jobId, {
        coverLetter,
      });
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const result = await appService.getCandidateApplications(
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const stats = await appService.getPipelineStats(
        req.params.jobId,
        req.user.companyId,
      );
      success(res, stats);
